...
class CocoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        bboxes = targets[:, :4]    # h == w default /h  for albumentations box format
        labels = targets[:, 4]
        labels = np.array(labels, dtype=np.int)
        if self.transform is not None:
            annotation = {'image': img, 'bboxes': bboxes, 'category_id': labels}
    ...
    if max_num_annots > 0:
        for idx, (annot, lab) in enumerate(zip(annots, labels)):
            if len(annot) > 0:
                ...
                # print(annot_padded[idx, :len(annot), :])
...
def test_coco_aug():
    ...
    img, bboxes, catg = dataone['image'], dataone['bboxes'], dataone['category_id']

    # -----test coco_collate for ablu xywh--> xyxy------
    epoch_iterator = iter(data.DataLoader(dataset, 1, shuffle=True, num_workers=0,
    ...
    for box in targets:
        l, t, l_right, t_down = map(int, box*640)    # coco format, after tp, keep x, y, w, h
        # l, t, l_w, t_h = map(int, box*640)    # ssd format, after tp, keep x, y, w, h
        # print(l, t, l_right, t_down, imgs.shape, type(imgs))
        img_rec = cv2.rectangle(imgs, (l, t), (l_right, t_down), (0, 0, 255), 2)
        # img_rec = cv2.rectangle(imgs, (l, t), (l + l_w, t + t_h), (0, 0, 255), 2)
    
    cv2.imwrite('rectangle_tp_point_collate_priobox_{}.jpg'.format(image_idx), img_rec)
    # -----test coco_collate for ablu xywh--> xyxy------
