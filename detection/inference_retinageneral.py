...
def inference(cfg, net, img_path):
    # assert net.scene == scene, 'scene are same'
    ...
    print(scale, 'ssssss', height, width)
    scale = scale.to('cuda')
    ...
    # if net.scene == 'face':    # anchor normalized, general is not
    #     boxes = boxes * scale / resize
    ...
    if landm is not None:
        ...
        landms = landms[inds]     # ignore low scores
        landms = landms[order]    # keep top-K before NMS
        landms = landms[keep]     # do NMS

        dets = np.concatenate((dets, landms), axis=1)    # concate boxes and landms
