...
# from data.coco_retinageneral import CocoDataset, coco_collate
...
def train():
    ...
    dataset = CocoDataset(training_dataset, set_name='annotations', transform=get_augumentation(phase='train', width=640, height=640, ft='coco'))
    ...
    for it in range(start_iter, max_iter):
        if it % epoch_size == 0:
            ...
            if (epoch % 80 == 0 and epoch > 0) or (epoch % 80 == 0 and epoch > cfg['decay1']):
                torch.save(net.state_dict(), os.path.join(save_weights, cfg['name']  + weights_label + 'epoch_' + str(epoch) + '.pth'))
            epoch += 1
